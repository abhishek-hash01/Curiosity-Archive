...
function WeekCard({ week }: { week: ReturnType<typeof useStore.getState>['weeks'][string] }) {
                ...
                <div className="flex flex-col gap-1">
                  {week.goals.map(g => (
                     <div key={g.id} className="flex items-start gap-2">
                       <CheckCircle2 className={`w-4 h-4 mt-0.5 shrink-0 ${g.completed ? 'text-primary' : 'text-muted-foreground/30'}`} />
